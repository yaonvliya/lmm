<template>
<div>
	<div id="module0" class="module-main module mshow0 product7" v-for="(items,indexs) in module1" :key="indexs">
       	<img class="img-title csmc" :src="items.headurl1">
		<div class="tab-main type_COMMON_STYLE_5" data-type="COMMON_STYLE_5">
			<div class="tab-list">
				<ul class="list">
					<li style="display: block;"  v-for="(item,index) in items.modu1" :key="index">
...
<script>
	export default{
		...
		created: function() {
				fetch("http://localhost:3000/module1")
					.then((res) => {
						return res.json();
					})
					.then((req) => {
